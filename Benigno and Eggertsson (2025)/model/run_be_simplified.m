...
% The supply shock shifts the kink point: the slack and tight pieces shift
% by different amounts (kappa_v vs kappa_v_tight), so continuity requires
% a new threshold where the two shifted pieces intersect:
%   y*' = y* - (kappa_v_tight - kappa_v)*(1-alpha)*q / (kappa_tight - kappa)
ystar_shifted = ystar - (kap_v_tight - kap_v) * (1-alpha) * q_shock / (kap_tight - kap);

as_shifted = as_shift_slack;
as_shifted(y_grid > ystar_shifted) = as_shift_tight(y_grid > ystar_shifted);
