...
var
    ...
    g       $\hat{g}$       (long_name='Demand shock state')
    nu      $\nu$           (long_name='Supply/cost-push shock state')
    e       $\hat{e}$       (long_name='Monetary policy shock state')
    chi     $\hat{\chi}$    (long_name='Labor participation shock state')
;

varexo
    eps_d   $\varepsilon_d$      (long_name='Demand shock innovation')
    eps_s   $\varepsilon_\nu$    (long_name='Supply shock innovation')
    eps_m   $\varepsilon_e$      (long_name='Monetary policy shock innovation')
    eps_chi $\varepsilon_\chi$   (long_name='Labor participation shock innovation')
;
...
model;
    ...
    // pi_t = kappa*(Y_t + alpha/omega * chi_t) + kappa_v * nu_t + kappa_beta * E_t(pi_{t+1})
    // Footnote 51: simplified model sets kappa_beta = beta = 1 (no long-run output-inflation tradeoff)
    [name='Phillips curve', relax='tight_labor']
    pi = kappa * (y + alpha_omega * chi) + kappa_v * nu + pi(+1);
    ...
    // Footnote 51: simplified model sets beta = 1
    [name='Phillips curve', bind='tight_labor']
    pi = -c_tilde + kappa_tight * (y + alpha_omega * chi) + kappa_v_tight * nu + pi(+1);
