...
$$\pi_t = \begin{cases} -c + \kappa^{tight}\hat{\theta}_t + \kappa_v^{tight}\left(\hat{v}_t + 
\hat\vartheta^{tight}_t\right) + \beta E_t \pi_{t+1} & \text{if } \hat{\theta}_t > \hat{\theta}_t^* \text{ (labor shortage)} \\ 
\kappa_w \hat{w}_{t-1} + \kappa \hat{\theta}_t + \kappa_v \left(\hat{v}_t + \hat{\vartheta}_t\right) + \kappa_\beta E_t \pi_{t+1} & \text{if } \hat{\theta}_t \leq \hat{\theta}_t^* \text{ (normal)} \end{cases}$$
...
3. **Lagged wages matter only in the normal regime**: During labor shortages, the Phillips curve is purely forward-looking (no $\hat{w}_{t-1}$ term). In normal times, wage persistence ($\kappa_w \hat{w}_{t-1}$) enters, creating inflation inertia.

**Sign convention note on $\kappa_v$:** Table B of the paper (2008–2023 sample) reports $\tilde{\kappa}_v = -0.0093$, which is negative. This reflects the sample composition of that specific period, not a structural negative relationship between cost-push shocks and inflation. In the structural model, $\hat{v}_t = \hat{\mu}_t - \hat{A}_t + (1-\alpha)\hat{q}_t$ is a cost-push measure (oil price $\hat{q}_t$ enters positively), and a positive $\hat{v}_t$ raises inflation. The Dynare implementation (`be_simplified.mod`) uses $\kappa_v = +0.0093$ (absolute value) treating $\nu_t$ as a cost-push shock with positive inflation pass-through, which is the structurally sensible convention. The 1960–2023 sample (column 1, Table B) gives $\kappa_v = +0.0447$, consistent with a positive supply shock coefficient.
...
- Taylor rule: $\hat{i}_t = \rho\hat{r}_t^e + \phi_\pi(\pi_t - \pi^*) + e_t$
